...
for w = 25  %: d : an
    ...
    for L1 = L1_start %: L1_interval : L1_end
        for L2 = L2_start %: L2_interval : L2_end
            ...
            pathToFolder = fullfile('F:\FreeFormAssets\CurvedSurface\', dictionaryname); % 锟斤拷取锟斤拷前锟斤拷锟斤拷目录锟斤拷锟斤拷锟侥硷拷锟斤拷锟斤拷锟斤拷锟斤拷铣锟斤拷锟斤拷锟铰凤拷锟?
            if ~exist(pathToFolder, 'dir') % 锟叫断革拷路锟斤拷锟斤拷锟角凤拷锟窖达拷锟斤拷同锟斤拷锟侥硷拷锟斤拷
                mkdir(pathToFolder); % 锟斤拷锟斤拷mkdir()锟斤拷锟斤拷锟斤拷锟斤拷锟侥硷拷锟斤拷
            end
            saveas(gcf, ['F:\FreeFormAssets\CurvedSurface\', dictionaryname, '\理想情况下.png'])
